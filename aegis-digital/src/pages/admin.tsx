...
export default function Admin() {
  ...
  const fetchApplications = async () => { setIsLoadingApps(true); try { const response = await fetch('/api/applications'); const data = await response.json(); if (data.success) setDbApplications(data.applications); } catch (error: any) { toast({ variant: 'destructive', title: 'Fetch Error', description: 'Could not sync records.' }); } finally { setIsLoadingApps(false); } };
  const fetchSubmissions = async () => { try { const response = await fetch('/api/admin/submissions'); const data = await response.json(); if (data.success) setSubmissions(data.submissions); } catch (error: any) { toast({ variant: 'destructive', title: 'Fetch Error', description: 'Could not sync submissions.' }); } };
  const fetchMessages = async () => { try { const response = await fetch('/api/admin/messages'); const data = await response.json(); if (data.success) setMessages(data.messages); } catch (error: any) { toast({ variant: 'destructive', title: 'Fetch Error', description: 'Could not sync messages.' }); } };
  const handleUpdateStatus = async (appId: string, newStatus: string) => { try { const response = await fetch(`/api/applications/${appId}/status`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ status: newStatus.toLowerCase() }), }); if (response.ok) { setDbApplications(prev => prev.map(app => app.id === appId ? { ...app, status: newStatus.toLowerCase() } : app)); toast({ title: `Status Updated`, description: `Application successfully ${newStatus.toLowerCase()}.` }); } } catch (e) { toast({ variant: 'destructive', title: 'Error', description: 'Failed to rewrite state.' }); } };
  const handleIssueCertificate = async (appId: string) => { if (!window.confirm("Issue certificate for this student?")) return; try { const response = await fetch(`/api/applications/${appId}/issue-certificate`, { method: 'PUT' }); if (response.ok) { setDbApplications(prev => prev.map(app => app.id === appId ? { ...app, certificateIssued: true } : app)); toast({ title: "Certificate Issued", description: "The student has been marked as certified." }); } } catch (e) { toast({ variant: 'destructive', title: "Error", description: "Failed to issue certificate." }); } };
  const handleUpdateSubmissionStatus = async (subId: string, newStatus: string) => { try { const response = await fetch(`/api/admin/submissions/${subId}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ status: newStatus.toLowerCase(), feedback: "" }), }); if (response.ok) { setSubmissions(prev => prev.map(sub => sub.id === subId ? { ...sub, status: newStatus.toLowerCase() } : sub)); toast({ title: 'Status Updated', description: `Submission successfully marked as ${newStatus.toLowerCase()}.` }); } } catch (e) { toast({ variant: 'destructive', title: 'Error', description: 'Failed to update submission status.' }); } };
  const handleDeleteSubmission = async (subId: string) => { if (!window.confirm("Delete this submission permanently?")) return; try { const response = await fetch(`/api/admin/submissions/${subId}`, { method: 'DELETE' }); if (response.ok) { setSubmissions(prev => prev.filter(s => s.id !== subId)); toast({ title: "Submission Deleted", description: "The assignment review row was purged." }); } } catch (e) { toast({ variant: 'destructive', title: 'Error', description: "Could not delete submission." }); } };
  const handleDeleteApplication = async (appId: string) => { if (!window.confirm("Are you absolutely sure you want to remove this student application from the system? This action is permanent.")) return; try { const response = await fetch(`/api/applications/${appId}`, { method: 'DELETE', }); const data = await response.json(); if (data.success) { setDbApplications(prev => prev.filter(app => app.id !== appId)); toast({ title: "Student Removed", description: "The record has been permanently deleted from the database." }); } } catch (err) { toast({ variant: 'destructive', title: "Deletion Failed", description: "Could not purge database row." }); } };
  ...
  const handleSaveEdits = async (e: React.FormEvent) => { e.preventDefault(); if (!editingApp) return; try { let currentDetails: any = {}; try { currentDetails = JSON.parse(editingApp.details); } catch(e) {} const updatedDetails = JSON.stringify({ ...currentDetails, phone: editPhone, country: editCountry, studentEmail: editStudentEmail }); const response = await fetch(`/api/applications/${editingApp.id}/edit`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ studentName: editStudentName, studentEmail: editStudentEmail, programName: editProgramName, details: updatedDetails }) }); const data = await response.json(); if (!data.success) { throw new Error(data.error || "Failed to update text records."); } if (editNewPassword.trim() !== '') { const passResponse = await fetch(`/api/applications/${editingApp.id}/reset-password`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ newPassword: editNewPassword }) }); const passData = await passResponse.json(); if (!passData.success) { throw new Error(passData.error || "Profile updated but password change failed."); } } setDbApplications(prev => prev.map(app => app.id === editingApp.id ? { ...app, studentName: editStudentName, studentEmail: editStudentEmail, programName: editProgramName, details: updatedDetails } : app)); setEditingApp(null); toast({ title: "Changes Saved", description: "Student dossier metrics and credentials updated successfully." }); } catch (err: any) { toast({ variant: 'destructive', title: "Update Error", description: err.message || "Failed to push modifications." }); } };

  if (!isAdminLoggedIn) { return ( <div className="min-h-screen flex items-center justify-center bg-background px-4"> <Card className="w-full max-w-md shadow-lg border border-border"> <CardHeader className="text-center"> <div className="mx-auto bg-primary/10 p-3 rounded-full w-fit mb-2"><Lock className="h-6 w-6 text-primary" /></div> <CardTitle className="text-2xl font-bold flex items-center justify-center gap-2"><Shield className="h-5 w-5 text-primary" /> Aegis Command Gate</CardTitle> </CardHeader> <CardContent> <form onSubmit={handleAdminLogin} className="space-y-4"> <Input type="text" placeholder="Admin Username" value={inputUsername} onChange={(e) => setInputUsername(e.target.value)} required /> <Input type="password" placeholder="Master Password" value={inputPassword} onChange={(e) => setInputPassword(e.target.value)} required /> <Button type="submit" className="w-full font-medium">Verify Credentials</Button> </form> </CardContent> </Card> </div> ); }
